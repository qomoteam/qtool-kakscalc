...
void KAKS::showParaInfo() {
	...
	cerr<<endl<<"Genetic code: "<<transl_table[2*(genetic_code-1)+1]<<endl;
	cerr<<"Please wait while reading sequences and calculating..."<<endl;
}
...
void KAKS::helpInfo() {
	...
	cerr<<endl;
	cerr<<"******************************************************************"<<endl;
	cerr<<"  Program: KaKs_Calculator"<<endl;
	cerr<<"  Version: "<<VERSION<<endl;
	cerr<<"  Description: Calculate Ka and Ks through model selection and model averaging."<<endl;
	cerr<<"******************************************************************"<<endl;
	cerr<<endl;

	cerr<<"Usage: Kaks_Calculator <options>"<<endl;
	...
	for (i=0; i<method_name.size(); i++) {
		...
		cerr<<"\t\t"<<method_ref[i].c_str()<<endl;
	}
	cerr<<"\t\t  ALL(including all above methods)"<<endl;

	cerr<<"\t-d\tFile name for details about each candidate model when using the method of MS"<<endl;
	cerr<<endl;
